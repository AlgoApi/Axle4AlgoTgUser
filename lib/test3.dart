...
class _MyHomePageState extends State<MyHomePage> with TickerProviderStateMixin {
  ...
  // Список слов-триггеров, которые вызывают окрашивание вкладки
  ...
  // Массив для отслеживания, окрашена ли вкладка
  ...
  Future<void> _loadAppConfigs() async {
    ...
    if (jsonString != null) {
      ...
      setState(() {
        ...
        _outputControllers =
            List.generate(_appConfigs.length, (_) => TextEditingController());
        _inputControllers =
            List.generate(_appConfigs.length, (_) => TextEditingController());
        _isTabHighlighted = List.filled(_appConfigs.length, false);
        _tabController = TabController(length: _appConfigs.length, vsync: this);
      });
  ...
  Future<void> _saveAppConfigs() async {
    ...
    final String jsonString =
        jsonEncode(_appConfigs.map((config) => config.toMap()).toList());
  ...
  Future<void> _startProcess(int index) async {
    ...
    try {
      ...
      setState(
          () {}); // Обновляем состояние для обновления числа активных приложений
    } catch (e) {
  ...
  void _addAppConfig(String path, String login, String phone, String text) {
    final newConfig =
        AppConfig(path: path, login: login, phone: phone, text: text);
    setState(() {
              ...
              ...List.generate(_appConfigs.length, (index) {
                return ListTile(
                  ...
                  trailing: IconButton(
                    ...
                    onPressed: () {
                      _removeAppConfig(index); // Удаление настройки и вкладки
